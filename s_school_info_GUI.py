    ...
    while item_try_remain >= 0:
        try:
            ...
            if find_school_name:
                ...
                label_latest_content.set("找到 " + school_name.strip() + " ！")
                root.update()
                last_good = input
                go_on = 0
                return html_data
...
def outputFile():
    ...
    label_output_content.set("已輸出 " + str(len(all_schoolinfo)) + " 項紀錄至CSV檔案")
    root.update()
...
def dataReport():
    ...
    if not error_found:
        label_report_content.set("沒有資料錯誤，準備輸出...")
    else:
        label_report_content.set(str(len(error_found)) + " 項錯誤資料未能修正，準備輸出")
    root.update()
          
def start():
    label_latest_content.set("開始收集資料，請稍候...")
...
label_report_content = StringVar()
Label(frame, textvariable=label_report_content, justify=LEFT).pack()
